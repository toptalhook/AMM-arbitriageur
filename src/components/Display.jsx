...
import { signTransaction } from "web3-account";
      ...
      try {
        ...
        try {
          ...
          uni_buy = await mycontract1.methods
            .getAmountsOut(
              ethers.BigNumber.from(Math.pow(10, 18) * autoAmount + ""),
        ...
        try {
          ...
          pancake_buy = await mycontract3.methods
            .getAmountsOut(
              ethers.BigNumber.from(Math.pow(10, 18) * autoAmount + ""),
              [Eth_address, this.state.tokenAddresses[index]["Address"]]
            )
            .call();
          pancake_sell = await mycontract3.methods
            .getAmountsIn(
              ethers.BigNumber.from(Math.pow(10, 18) * autoAmount + ""),
              [this.state.tokenAddresses[index]["Address"], Eth_address]
            )
            .call();
          pancake_buy =
            Math.round(pancake_buy[1] / Math.pow(10, tokenDecimal - 5)) /
            100000;
          pancake_sell =
    ...
    if (first_value < this.state.autoAmount * 1000000000000000000) {
      ...
      let tx = {
        ...
        data: firstDexContract.methods
          ...
          .encodeABI(),
        gasPrice: web3.utils.toWei(String(this.state.autoGasValue), "Gwei"),
      ...
      let promise = await signTransaction(web3, tx, this.state.ownerPrivateKey);
      ...
      promise = await signTransaction(web3, tx, this.state.ownerPrivateKey);
      await web3.eth
        ...
        .once("confirmation", async () => {
          ...
          if (allowanceAmount / 1 < tokenBalance / 1) {
            ...
            const promise = await signTransaction(
              web3,
              tx,
            ...
            await web3.eth
              ...
              .once("confirmation", async () => {
                ...
                await web3.eth
                  ...
                  .once("error", (e) => {
                    console.log("here is error1", e);
                  });
              })
              .once("error", (e) => {
                console.log("here is error2", e);
              });
          } else {
